...
select DISTINCT department_id, job_id
 ...
 -- where, order by
 /* 샘플 - hr부서 예산 편셩 문제로 급여 정보 보고서를 작성한다. employees 에서 salary가 7000 ~ 10000달러 범위 의외의 사람의
...
/* 문제1 - 사원의 last_name 중 e나 o 글자를 포함한 사원을 출력하시오. 이때 컬럼명은 e AND o Name 이라고 출력하시오.(10행)
*/
...
/* 문제1 - 사원이 소속된 부서별 급여 합계, 급여 평균, 급여 최대값, 급여 최소값을 집계.
            ...
            단, 부서에 소속되지 않는 사원은 정보에서 제외, 출력시 머리클은 아래처럼 별칭으로 처리(11개행)
*/
select department_id
	 , concat('$', format(round(sum(salary),0),0)) as 'Sum Salary'
     , concat('$', format(round(avg(salary),1),1)) as 'Avg Salary'	-- round(컬럼, 1), 소수점 1자리에서 반올림, format(값, 1) 소수점표현 및 100단위 ,표시
...
-- 서브쿼리
/* 문제 3 - 사원들의 지역별 근무현황을 조회. 도시 이름이 영문 'O'로 시작하는 지역에 살고
