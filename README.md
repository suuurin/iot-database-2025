...
- SQL 기초
    ...
    - DML 중 INSERT, UPDATE, DELETE : [SQL](./day03/db02_dml_쿼리.sql)
        ...
        VALUES (값리스트); -- VALUE는 SQL 표준이 아님(MySQL, PostgreSQL 등에서만 사용가능)
...
- 데이터베이스 프로그래밍
    ...
    - 사용자 정의 함수 : [SQL](./day05/db03_사용자정의함수.sql)
        - 리턴 키워드 사용

- 데이터베이스 연동 프로그래밍
    - PyMySQL 모듈사용
    - 파이썬 DB연동 콘솔 : [노트북](./day05/db05_파이썬_DB연동.ipynb)
    - 파이썬 DB연동 웹 : [Python](./day05/index.py)
...
- 정규화 : [SQL](./day06/db03_정규화_이상현상.sql)
    ...
    - 정규화 - 이상현상이 발생하지 않도록 릴레이션(개체, 테이블)을 분해하는 과정
        ...
        - 제2정규화 - 모든 랄레이션의 속성이 완전함수종속을 하는 정규화
...
- Workbench Tip
    ...
    - 메뉴 `Edit > Preferences > SQL Editor > SQL Execution 에서 Limit Rows Count`를 조V절
...
- 인덱스 실습 : [SQL](./day07/db01_인덱스연습.sql)
    - 500만건 조회시 price로 검색
        - 인덱스가 없으면 0.67초 소요
        - 인덱스를 걸면 0.06초 소요
- 데이터베이스 관리와 보안 : [SQL](./day07/db02-db관리.sql)
...
- tkinter DB연동 GUI앱 개발 : [Python](./day09/students_regapp.py)
    ...
    2. madang DB에 students 테이블 생성 및 더미 데이터 추가
        ```sql
